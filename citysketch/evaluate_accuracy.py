...
def pick_ckpt(root: Path):
    sup = root/'ckpt_sup.pt'; unsup = root/'ckpt_structure_first.pt'
    if sup.exists(): return 'sup', sup
    if unsup.exists(): return 'unsup', unsup
    return 'none', None
...
ap.add_argument("--topk", type=int, default=20)
    ...
    which, ckpt = pick_ckpt(Path(args.graphs_root))
    model = StructureFirstGNN(node_in=node_in, edge_in=edge_in).to(args.device)
    head = None
    if which=='sup':
        ck = torch.load(ckpt, map_location=args.device)
        model.load_state_dict(ck['backbone'])
        head = PairwiseHead(dim=model.readout[-1].out_features if hasattr(model.readout[-1],'out_features') else 128).to(args.device)
        head.load_state_dict(ck['head'])
        print('Loaded supervised checkpoint:', ckpt)
    elif which=='unsup':
        model.load_state_dict(torch.load(ckpt, map_location=args.device))
        print('Loaded unsupervised checkpoint:', ckpt)
    else:
