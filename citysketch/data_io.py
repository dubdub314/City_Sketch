...
def read_shapefiles(sample_dir: Path) -> Dict[str, gpd.GeoDataFrame]:
    ...
    for shp in sample_dir.rglob("*.shp"):
        ...
        if NAME_FIELD not in gdf.columns and "name" in cols:
            gdf = gdf.rename(columns={cols["name"]: NAME_FIELD})
        if TYPE_FIELD not in gdf.columns and "type" in cols:
            gdf = gdf.rename(columns={cols["type"]: TYPE_FIELD})
...
def load_manifest(manifest_path: Path):
    ...
    recs = [json.loads(Path(p).read_text(encoding='utf-8')) for p in paths]
    return recs
